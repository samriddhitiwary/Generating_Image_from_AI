...
const Notic = () => {
  ...
  const AI_MODEL = ["VAI Image Art Dall-e-v2", "AI Image Art Dall-e-v3"];

  const [aiModel, setAiModel] = useState("AI Image Art Dall-e-v2");

  const changeModel = (Mmodel) => {
    setAiModel(model);
    ...
    >
      {openNotic && (
        ...
        >
          ...
          >
            <div
              className="bf-zinc-800 p-1 drop-shadow-md border
             border-zinc-700 rounded-md"
              style={{
                position: "relative",
                flex: "1 1 0%",
                overflow: "auto",
              ...
              <div className="select-none">
                {AI_MODEL.map((model, index) => (
                  <div
                    ...
                    cursor-pointer hover:bg-zinc-700 rounded text-xs relative focus:outline-none "
                  >
                    <span>{model}</span>
                    {aiModel == model && (
      ...
      <div className="w-full flex flex-col items-center">
        <div className="w-full px-3 pt-3 flex space-x-1 items-center justify-center pr-12">
          <button
            className="flex cursor-pointer select-none hover:bg-zinc-900
           hover:bg-opacity-50 pl-3 pr-3 pr-2.5 py-2 items-center justify-center space-x-1.5
           rounded focus:outline-none text-xs text-zinc-200"
            type="button"
            onClick={() =>
