...
const Header = () => {
  ...
  const navList = [
    { menu: "Home", link: "/" },
    { menu: "Generate", link: "/aperture" },
    { menu: "History", link: "/history" },
    { menu: "Likes", link: "/likes" },
  ...
  return (
    <div className="relative z-50">
      {/* Header Navigation */}
      <div
        className="fixed w-screen bottom-0 sm:top-0 z-50 flex flex-row items-center justify-between 
        backdrop-blur bg-opacity-80 border-t sm:border-b border-opacity-50 text-sm select-none 
      ...
      >
        ...
        <div className="flex relative items-center h-full -mt-1 w-full sm:w-auto">
          {navList.map((menu, index) => (
            <a
              key={index}
              className="flex flex-row items-center cursor-pointer h-full px-2 py-1.5 pb-0 
              justify-center transition-all flex-1"
              href={menu.link}
        ...
        {/* User Interaction */}
        <div className="w-32 h-full flex items-center justify-end mr-6">
          <button
            className="flex items-center justify-center h-8 rounded-md opacity-90 hover:brightness-110 
            px-4 text-xs md:text-sm bg-gradient-to-t from-indigo-800 to-indigo-600 drop-shadow font-medium"
            onClick={() => setOpenSetting(!openSetting)}
          >
            Get Started
          </button>
